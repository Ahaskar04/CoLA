    ...
    for name, s, e in (('scoring loop', _SCORE_START, _SCORE_END),
        ...
        if _block(cola, s, e) != _block(mine, s, e):
            raise SystemExit(f'{name} differs from {COLA_SOURCE} -- regenerate '
                             f'eval_octo_marker.py before trusting a number from it')
...
def evaluate_octo_marker(checkpoint_a, checkpoint_b, n_episodes=150,
    ...
    print('=' * 64)
    print('OCTO HIDDEN-MARKER HANDOVER EVALUATION (decentralised)')
    print('=' * 64)
    print(f'Arm a: {checkpoint_a}')
    print(f'Arm b: {checkpoint_b}')
    print(f'Episodes: {n_episodes} (seed offset {seed_offset})  |  chance = 33.3% (three trays)')
    print('Messages: none -- two independent policies, no channel')
    ...
    if centralised:
        ...
        n_out = policy_a.action_stats['mean'].shape[-1]
        assert n_out == 14, (
        ...
        policy_b = FinetunedOctoPolicy(checkpoint_b, seed=seed + seed_offset + 1)
        assert policy_a.meta.get('arm') == 'a' and policy_b.meta.get('arm') == 'b', (
            'checkpoint-a must be an arm-a policy and checkpoint-b an arm-b policy')
    ...
    results = {
        'model': ('octo finetuned, centralised (one 14-d policy, both wrists)'
                  if centralised else 'octo-small-1.5 finetuned, decentralised'),
        'checkpoints': ({'centralised': str(checkpoint_a)} if centralised
                        else {'a': str(checkpoint_a), 'b': str(checkpoint_b)}),
...
if __name__ == '__main__':
    ...
    p.add_argument('--checkpoint-a')
    p.add_argument('--checkpoint-b')
    ...
    p.add_argument('--centralised', action='store_true',
                   help='checkpoint-a is ONE 14-d policy driving both arms; '
                        'checkpoint-b is then unused. The free-information-'
                        'sharing upper bound: arm B cannot see the marker '
                        'from its own wrist, but a single network holding '
                        "arm A's view can route on it.")
    ...
    if a.merge:
        ...
        if a.centralised:
            assert a.checkpoint_a, '--checkpoint-a required'
        else:
            assert a.checkpoint_a and a.checkpoint_b, '--checkpoint-a/-b required'
        evaluate_octo_marker(a.checkpoint_a, a.checkpoint_b, n_episodes=a.episodes,
                             ...
                             seed=a.seed, centralised=a.centralised)
