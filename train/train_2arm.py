"""Train the two-arm CoLA adapters on cached Octo features.

The whole split lives in device memory and each step is one jitted function.

Expects, per split:
    {feat_dir}/{split}_features_{a,b}.npy   (N, 768)
    {cache_dir}/{split}_actions_{a,b}.npy   (N, 7)
    {cache_dir}/{split}_states_{a,b}.npy    (N, 7), unless --no_proprio

Usage:
    python train_2arm.py --cache_dir CACHE --feat_dir FEATS --run_dir RUN
"""
...
from cola.model import (COLAModel, GRIPPER_IDX, CHUNK_SIZE, ACTION_DIM,  # noqa: E402
                        DIFFUSION_STEPS)
from cola.dataset import COLADataset  # noqa: E402

CACHE_DIR = str(REPO / 'data' / 'cache' / 'handover_2arm')
FEAT_DIR = str(REPO / 'data' / 'features' / 'handover_2arm')
SCENE_XML = str(REPO / 'envs' / 'handover_2arm' / 'scene.xml')
RUN_DIR = REPO / 'runs' / 'handover_2arm'
...
class DeviceDataset:
    ...
    def __init__(self, cache_dir, feat_dir, split, use_proprio,
        ...
        # Shared overhead view (from extract_features_2arm.py --overhead).
        feat_o = None
        if use_overhead:
...
def train_cola(
    ...
    cfg = {
        ...
        # Channel transfer (None / False for ordinary runs).
        'init_channel_from': str(init_channel_from) if init_channel_from else None,
        'freeze_channel': bool(freeze_channel),
    }
    ...
    model = COLAModel(use_proprio=use_proprio, split_gripper=True,
                      ...
                      use_diffusion=use_diffusion,
                      diffusion_unet=diffusion_unet, unet_dims=unet_dims, message_dim=message_dim,
                      seed=seed)
    if not use_messages:
    ...
    base_opt = optax.chain(optax.clip_by_global_norm(1.0), optax.adam(schedule))
    params = model.params

    # Channel transfer: the channel is both proprio encoders, both message
    # encoders and both decoders.
    channel = [k for k in params if k.startswith(('encoder_', 'decoder_', 'proprio_'))]
    if init_channel_from:
        with open(init_channel_from, 'rb') as f:
            src = pickle.load(f)
        sp, scfg = src['params'], src.get('config', {})
        if bool(scfg.get('use_overhead')) != bool(use_overhead):
            raise ValueError(f'source use_overhead={scfg.get("use_overhead")} '
                             f'!= target {use_overhead}: encoder input widths differ')
        if bool(scfg.get('use_wrist', True)) != bool(use_wrist):
            raise ValueError(f'source use_wrist={scfg.get("use_wrist", True)} '
                             f'!= target {use_wrist}: encoder input widths differ')
        if src.get('use_messages') is False or scfg.get('use_messages') is False:
            raise ValueError('source checkpoint was trained with messages SEVERED; '
                             'its channel never received gradient')
        new = dict(params)
        for k in channel:
            if k not in sp:
                raise ValueError(f'source has no {k}')
            a, b = jax.tree_util.tree_leaves(params[k]), jax.tree_util.tree_leaves(sp[k])
            if len(a) != len(b) or any(x.shape != y.shape for x, y in zip(a, b)):
                raise ValueError(f'{k}: shape mismatch source vs target')
            new[k] = jax.tree_util.tree_map(jnp.asarray, sp[k])
        params = type(params)(new) if not isinstance(params, dict) else new
        model.params = params
        logger.info(f'  channel initialised from {init_channel_from}: {channel}')
    if freeze_channel:
        def label(path, _):
            return 'frozen' if path[0].key in channel else 'train'
        optimizer = optax.multi_transform(
            {'train': base_opt, 'frozen': optax.set_to_zero()},
            lambda p: jax.tree_util.tree_map_with_path(label, p))
        n_fz = sum(x.size for k in channel for x in jax.tree_util.tree_leaves(params[k]))
        n_all = sum(x.size for x in jax.tree_util.tree_leaves(params))
        logger.info(f'  FROZEN: {channel} ({n_fz:,} params); training '
                    f'{n_all - n_fz:,} of {n_all:,}. Grad-norm clip now spans '
                    f'the trainable heads only.')
    else:
        optimizer = base_opt
    ...
    try:
        for epoch in range(num_epochs):
            ...
            payload = {
                ...
                'use_wrist': use_wrist,
                'split_gripper': True,
                # Top-level keys, read by the evaluators before building the model.
                'use_messages': use_messages,
                'message_dim': message_dim,
                'config': cfg,
                # Data provenance.
...
if __name__ == '__main__':
    ...
    ap.add_argument('--overhead_only', action='store_true',
                    help='use only the shared overhead camera, no wrist cameras '
                         '(self_dim 768). Requires extract_features_2arm.py --overhead.')
    ...
    ap.add_argument('--message_dim', type=int, default=64,
                    help='CHANNEL BOTTLENECK width d_m. 64 is the default every '
                         'existing result used. Sweeping this (16/32/64/128) '
                         'measures how much capacity the inter-agent channel '
                         'actually needs: on the hidden-marker task the message '
                         'must carry one of three colours, which is ~1.6 bits, '
                         'so a collapse only at very small widths would say the '
                         'channel is near-minimally used, while a smooth decline '
                         'would say it carries more than the colour. Encoder and '
                         'decoder both rebuild at this width, so the parameter '
                         'count changes with it -- unlike --no_messages, this is '
                         'NOT a capacity-matched ablation.')
    ...
    ap.add_argument('--init_channel_from', default=None,
                    help='checkpoint whose encoder/decoder/proprio encoders '
                         'initialise this run (shapes must match exactly)')
    ap.add_argument('--freeze_channel', action='store_true',
                    help='never update encoder/decoder/proprio encoders; only '
                         'the action heads train. Without --init_channel_from '
                         'this freezes the channel at its random init.')
    args = ap.parse_args()
    ...
    train_cola(
        ...
        seed=args.seed,
        init_channel_from=args.init_channel_from,
    ...
    print('=' * 60)
    print('Next: python eval/eval_2arm.py '
