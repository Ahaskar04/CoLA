...
from cola_architecture import (COLAModel, GRIPPER_IDX, CHUNK_SIZE, ACTION_DIM,
                               DIFFUSION_STEPS)
from cola_dataset import COLADataset

CACHE_DIR = '/scratch/users/ntu/ahaskar0/v1/cola-research-scratchdata/cache_aloha_handover_v2/'
FEAT_DIR = '/scratch/users/ntu/ahaskar0/v1/cola-research-scratchdata/features_aloha_handover_v2/'
...
def train_cola(
    ...
    cfg = {
        ...
        # Channel-transfer provenance. None / False for every ordinary run.
    ...
    # CHANNEL TRANSFER. "The channel" is everything upstream of the action
    # heads that shapes the message: both proprio encoders (they are part of
    # the encoder's input, so relearning them would change what the frozen
    # encoder sees), both message encoders and both decoders.
    ...
    try:
        for epoch in range(num_epochs):
            ...
            payload = {
                ...
                'use_messages': use_messages,
                # TOP LEVEL like use_messages: cola_eval_marker.py must know the
                # channel width BEFORE it builds the model, and a mismatch is a
                # silent parameter-shape failure rather than a clean error.
                'message_dim': message_dim,
                'config': cfg,
