...
evaluator code (checked by check_criterion()).
...
class ColaFrozenPi05Policy:
    """Both arms' chunks for one control step: frozen pi0.5 features -> CoLA."""

    def __init__(self, model_path, cache_dir, seed=0):
        import pickle
        ...
        with open(model_path, 'rb') as f:
            ckpt = pickle.load(f)
        cfg = ckpt.get('config', {})
        self.use_messages = ckpt.get('use_messages', cfg.get('use_messages', True)) is not False
        self.use_proprio = bool(ckpt.get('use_proprio', cfg.get('use_proprio', False)))
        self.split_gripper = bool(ckpt.get('split_gripper', True))
        assert not ckpt.get('use_overhead', cfg.get('use_overhead', False)), (
            'this checkpoint used the overhead camera; the marker runs are wrist-only')
        assert cfg.get('use_diffusion') and cfg.get('diffusion_unet'), (
            f'expected CoLA diffusion U-Net head, got {cfg}')

        self.cola = COLAModel(use_proprio=self.use_proprio, split_gripper=self.split_gripper,
                              use_overhead=False, use_diffusion=True, diffusion_unet=True,
                              unet_dims=cfg.get('unet_dims'))
        self.cola.params = ckpt['params']
        self.pi05 = Pi05Features(cache=cache_dir_pi05)
        self.jax = jax
        self.rng = jax.random.PRNGKey(seed)
        self.denormalise = load_action_stats(cache_dir)
        self.normalise_state = load_state_normaliser(cache_dir)
        assert not (self.use_proprio and self.normalise_state is None), (
            'checkpoint wants proprio but state_stats.json is missing')
        print(f"   CoLA (frozen pi0.5, {FEATURE_DIM}-d features) from {model_path}")
        print(f"   epoch {ckpt.get('epoch')} val {ckpt.get('val_loss'):.6f} | "
              f"messages {'ON' if self.use_messages else 'OFF'} | proprio "
              f"{'on' if self.use_proprio else 'off'}")
    ...
    def act_both(self, scene, renderer):
        ...
        state_a, state_b = read_state(data, scene['a']), read_state(data, scene['b'])
        feat_a = self.pi05.features(image_a, state_a, 'a')
        feat_b = self.pi05.features(image_b, state_b, 'b')
        proprio_a = self.normalise_state(state_a, 'state_a') if self.use_proprio else None
        proprio_b = self.normalise_state(state_b, 'state_b') if self.use_proprio else None

        combined_a, combined_b = self.cola.forward_from_features(
            feat_a, feat_b, use_messages=self.use_messages,
            proprio_a=proprio_a, proprio_b=proprio_b)
        self.rng, k_a, k_b = self.jax.random.split(self.rng, 3)
        ...
        for chunk, arm in ((chunk_a, 'action_a'), (chunk_b, 'action_b')):
            ...
            chunk = self.denormalise(chunk, arm)
            chunk[:, 6] = grip
            out.append(chunk)
        return out[0], out[1]
...
def evaluate_cola_pi05_marker(model_path, cache_dir=CACHE_DIR, n_episodes=150,
    ...
    print('=' * 64)
    print(f'Model: {model_path}  |  cache {cache_dir}')
    ...
    results = {
        'model': 'CoLA on FROZEN pi0.5, decentralised with 64-d channel',
        'checkpoints': {'model': str(model_path)},
...
if __name__ == '__main__':
    ...
    if a.merge:
        ...
        evaluate_cola_pi05_marker(a.model, cache_dir=a.cache_dir, n_episodes=a.episodes,
